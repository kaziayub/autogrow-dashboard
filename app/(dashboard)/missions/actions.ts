...
export async function createMission(formData: FormData) {
  ...
  const agent_name = String(formData.get("agent_name") ?? "Executive").trim();
...
export async function runMissionAction(id: string, title: string) {
  ...
  // Fetch the mission to read assigned agent
  ...
  const assignedAgent = mData?.agent_name || "Executive";
  ...
  // 4. Also try VPS direct webhook as fallback
  const VPS_URL = process.env.VPS_WEBHOOK_URL || "http://63.180.69.67:3005/api/trigger-agent";
  fetch(VPS_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ agent_name: assignedAgent, action: "run_mission", payload: { mission_id: id, title } }),
